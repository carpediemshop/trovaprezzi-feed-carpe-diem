...
async function getTargetShop() {
  if (FORCED_SHOP_DOMAIN) {
    return FORCED_SHOP_DOMAIN;
  }

  const latestOfflineSession = await prisma.session.findFirst({
    where: {
      isOnline: false,
      accessToken: {
        not: "",
  ...
  if (!latestOfflineSession?.shop) {
    throw new Error(
      "Nessuno shop disponibile nel database sessioni. Installa prima l'app su uno shop Shopify."
    );
  }

  return latestOfflineSession.shop;
}
...
export async function loader() {
  const targetShop = await getTargetShop();
  const { admin } = await unauthenticated.admin(targetShop);

  const shop = await fetchShopInfo(admin);
  const shopName = normalizeText(shop?.name) || targetShop;
  const primaryDomainUrl = normalizeUrl(shop?.primaryDomain?.url);

  if (!primaryDomainUrl) {
    throw new Error(
      "Dominio primario Shopify non disponibile. Controlla la configurazione dello shop."
    );
  }
